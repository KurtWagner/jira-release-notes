...
class Jira {
  ...
  getReleasedIssues({ versionName }) {
    ...
    const jql = `fixVersion = "${versionName}" AND type NOT IN (Sub-task, Epic)`;
  ...
  _request(method, endpoint, body) {
    return rp(
      ...
      // console.log(response.issues.map(issue => issue.fields.issuetype));
      // console.log(response.key);
      // console.log(response.)
      const issues = response.issues.map(issue => {
        const rawReporter = issue.fields.reporter;
        const reporter = rawReporter
          ? new Account({
              emailAddress: rawReporter.emailAddress,
              id: rawReporter.accountId,
              displayName: rawReporter.displayName
            })
          : null;

        const rawAssignee = issue.fields.assignee;
        const assignee = rawAssignee
          ? new Account({
              emailAddress: rawAssignee.emailAddress,
              id: rawAssignee.accountId,
              displayName: rawAssignee.displayName
            })
          : null;
      ...
      for (const issue of response.issues) {
        // console.log(issue.fields.versions);
      }
